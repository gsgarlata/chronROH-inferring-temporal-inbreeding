...
AncientPopSize = 500 #Ancestral population size
...
TimeSizeChange = 200 #Time of population size change (backward in time)
RecentPopSize = 100 #Recent population size (before population size change, backward-in-time)
...
out_dir_name = 'Panmictic_ancientNe' + str(AncientPopSize) + '_PopSizeChangeTime' + str(TimeSizeChange) + '_recentNe' + str(RecentPopSize) + '_SampleSize' + str(SampleSize) + '_rbp' + str(rbp) + '_GenomeLength' + region_length_expFormat
...
for simID in range(1,(n_sims + 1),1):
    ...
    if os.path.isfile(dict_simulation_file):
        ...
        #Define the recent population with size "RecentPopSize"
        demog_model.add_population(initial_size=RecentPopSize)
        
        #Define the ancient population with size "AncientPopSize" and the time of population
        #size change at "TimeSizeChange".
        demog_model.add_population_parameters_change(time=TimeSizeChange, initial_size=AncientPopSize)
        
        #Run the simulation in msprime using the custom function "run_simulationIBDpanmicticPopSizeChangeMultipleSamplingTimes" 
        res_onesim = PanmicticMsprime.run_simulationIBD_PopSizeChange(SamplingTime_list = SamplingTime_list, SampleSize = SampleSize, demography = demog_model, 
